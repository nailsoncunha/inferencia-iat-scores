...
iat %>% 
    group_by(sex) %>% 
    summarise(media = mean(d_art), desvio = sd(d_art), n())
```
...
Em média, as mulheres que participaram do experimento tiveram uma associação implícita (medida pelo IAT) com a matemática **negativa** e **forte** (média 0.62341987, desv. padrão 0.4317694, N = 57). Homens tiveram uma associação **negativa** com a matemática, entretanto **menor** que a das mulheres (média 0.09923318, desv. padrão 0.5218736, N = 24). Houve portanto uma **considerável** diferença entre homens e mulheres (diferença das médias -0.5241867, 95% CI [-0.7721173, -0.2966195]). A partir desta amostra, estimamos que mulheres tem uma associação negativa mais forte com uma diferença que provavelmente está entre 0.2 e 0.7, porém não é claro se essa diferença é grande ou moderada. Pode ser necessário coletar mais dados para determinar se a diferença é relevante.
